...
FPS = 110.0 #estimation. The video appears slowed
...
def estimateVelocity(pos0, pos1, normalized=False):
    if normalized: #Shouldn't this ben if not normalized? -Victor
...
def distance4D(p, q, r, s):
    ...
    return dist
def predictionDistance(ballCenter, clusterCenter, ballVelocity):
    dist = math.sqrt( (ballCenter[0] + ballVelocity[0]*timeStepSize - clusterCenter[0])**2 + (ballCenter[1] + ballVelocity[1]*timeStepSize - clusterCenter[1])**2)
    return dist
...
def findBallsInImage(image, ballCenters, ballVelocities, kalman):
    ...
    if len(points[0]) >= numBallsToFind:
        ...
        if kalman:
            ...
            #filter(lambda x,y: y if x %4==1), zip(range(len(predicted_state)), predicted_state))
            ...
            ballCenters = predicted_ball_centers
            ballVelocities = predicted_ball_velocities
        ...
        for pair in itertools.product(zip(ballCenters, ballVelocities), centers):
            ...
            #print ballCenter
            #print center
            distance = distance4D(ballCenter, center, theoreticalVelocity, theoreticalVelocity)#NOTE this is not the right function!! Change it!
            #distance = predictionDistance(ballCenter, center, ballVelocity)
            pairings.append([pair[0], [pair[1], theoreticalVelocity, ballVelocity], distance])
        ...
        for pairing in sorted_pairings:
            ...
            if p not in [x[1][0] for x in min_matches]:
                ...
                temp_output_data.append(distance2D(pairing[1][1], pairing[1][2]))
                new_measurements.append(pairing[0][0]) #the new state of the measurements.

        # min_matches now contains the minimum matches for this set
        if kalman:
            new_measurements = sum( sum(new_measurements, ()), () ) #flattens the array twice.
...
def createFilter(balls):
    ...
    # def __f(n):
    #     return 4*math.floor(n/2)+n%2 #returns the index which should have a 1 in the measurementMatrix.
    # def __g(n):
    #     local_var = np.zeros(4*balls)
    #     v = __f(n)
    #     local_var[int(v)] = 1
    #     return local_var
    H = [[1,0,0,0],
...
def main():
    ...
    cv2.namedWindow('hsv')
    firstFrame = getFrame(cap) #use the first frame to set the color bonuds.
    ...
    #filterList = [copy.deepcopy(kalman) for i in range(numBalls)] #create one filter for each ball.
    while(cap.isOpened()):
        frame = getFrame(cap)
        if frame is None:
            break
        ...
        for color, ballIndices in [('red',[0,1,2,3])]:
            ...
            #print np.array(ballCentersToPair[0])
            ...
            if hsvColorBounds != (np.array([-5, 250, 235], np.uint8),np.array([5,  260, 275], np.uint8)):
                matches = findBallsInImage(thresholdImage, ballCentersToPair, ballVelocitiesToPair, kalman)
                print matches
            else:
