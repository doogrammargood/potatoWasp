...
def probBallGivenPixel(circle_hist, frame_hist): #circle_hist is the prob. of pixel color given that the pixel is in the circle.
    ...
    #print circle_hist
    for c in range(3):
        normalized_color = [ (circle_hist[c][i] + 1) / (frame_hist[c][i] + 255) for i in range(len(frame_hist[0]))] #We leave out the factor of Pr(pixel in circle). It is a constant.
...
while(True):
    ...
    #foreground = frame_copy
    circles = cv2.HoughCircles(foreground,cv2.HOUGH_GRADIENT,1,20,
                            param1=1,param2=20,minRadius=0,maxRadius=30)
